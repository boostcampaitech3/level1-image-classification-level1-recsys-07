...
class CustomMaskDataset(Dataset):
    ...
    def __init__(self, data_dir, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246), val_ratio=0.2):
        self.data_dir = data_dir
        self.df = pd.read_csv('/opt/ml/input/data/train/info.csv')
        self.df['path'] = data_dir + '/' + self.df['ImageID']
        self.mean = mean
        self.std = std
        self.val_ratio = val_ratio

        self.classes = torch.tensor(self.df['label'])
        self.image_path = self.df.path.values
    ...
    def split_dataset(self) -> Tuple[Subset, Subset]:
        n_val = int(len(self) * self.val_ratio)
        n_train = len(self) - n_val
        path_list = self.image_path
        groups = self.id
        gss = GroupShuffleSplit(n_splits=1, train_size=(n_train/len(self)))
        labels = self.classes
        for train_idx, val_idx in gss.split(path_list, labels, groups) :
    ...
    def __getitem__(self, index):
        ...
        image = self.read_image(index)
        multi_class_label = self.classes[index]
...
if __name__ == '__main__':
    ...
    dataset = CustomMaskDataset(data_dir='/opt/ml/input/data/train/images')
    ...
    labels_np = torch.clone(total_labels).numpy()
    _, count = np.unique(labels_np, return_counts=True)
    print(count)