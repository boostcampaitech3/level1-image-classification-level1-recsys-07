...
def train(data_dir, model_dir, args):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            r = np.random.rand(1)
            if r < mixup_prob :
                inputs, labels_a, labels_b, lam = mixup_data(inputs, labels.view(-1,1), device)
                inputs = inputs.to(device)
                labels_a = labels_a.squeeze().to(device)
                labels_b = labels_b.squeeze().to(device)
            ...
            # train_outputs.append(outs)

            # labels_a = torch.nn.functional.one_hot(labels_a, num_classes=outs.size(1)) * lam
            # labels_b = torch.nn.functional.one_hot(labels_b, num_classes=outs.size(1)) * (1-lam)

            # one_hot_labels = labels_a + labels_b

            # train_targets.append(one_hot_labels)
            ...
            if (idx + 1) % args.log_interval == 0:
                ...
                # train_rmse = np.sqrt(mean_squared_error(train_outputs_np, train_targets_np))
                
                current_lr = get_lr(optimizer)
                # print(
                #     f"Epoch[{epoch}/{args.epochs}]({idx + 1}/{len(train_loader)}) || "
                #     f"training loss {train_loss:4.4} || training rmse {train_rmse:4.4} || lr {current_lr}"
                # )
                print(
                    f"Epoch[{epoch}/{args.epochs}]({idx + 1}/{len(train_loader)}) || "
